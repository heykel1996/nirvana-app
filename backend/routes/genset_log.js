...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Genset Log Get Error:', error);
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    const {
      ...
      oil_pressure,
      ampere_accu,
      pipa_bahan_bakar_checked,
      filter_checked,
      visual_inspection,
      air_filter_connection,
      air_filter_pipe,
      air_filter_replace,
      air_filter_clean,
      mesin_bersih,
    ...
    console.log('Genset Log Create - User ID from token:', user_id);
    ...
    const [newLog] = await db.query(
      'SELECT * FROM genset_log WHERE id = ?',
      [result.insertId]
    );
    ...
    res.status(201).json({
      success: true,
      message: 'Genset log created successfully',
      data: newLog[0]
    });

  } catch (error) {
    console.error('Genset Log Create Error:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Genset Log Delete Error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
